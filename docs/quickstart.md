...
| `rename <old> <new>` | Rename an REU filesystem file |
...
screens. The REU filesystem keeps named blocks (`save <name> <start> <end>`,
`load <name> <addr>`, `del <name>`, `rename <old> <new>`, `type <file>`,
`dir`, `format`, `fsinfo`) that persist across reboots.
