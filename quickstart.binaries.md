...
`godzi` is deliberately **not** re-runnable — it holds the SID for its whole session and
drives every sprite register, so a second `run godzi` gets `? already running` rather than
two copies fighting over the chip; it also refuses with `? sprites busy` if another task is
driving sprites, since its two switchable looks could not share them.
...
| `sid.lib` | `sidplay`, `gortris`, `godzi` | COMPUTE!'s Enhanced Sidplayer: `kSidPlay` (play a `.mus` image from RAM — blocking) and `kSidHush` (silence, and stop a running player). ⚠️ The **caller** owns the tune buffer and must declare ZP `$02-$0A`, the player's scratch |
